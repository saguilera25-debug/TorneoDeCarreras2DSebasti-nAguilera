...
public class AStarNode
{
    ...
    public AStarNode parent; // 🔥 IMPORTANTE (para reconstruir el camino)
    ...
    {
        // 🔥 Eliminamos el bloqueo (muy importante)
        // if (isCostCalculated) return;

        gCostDistanceFromStart =
            Mathf.Abs(gridPosition.x - aiPosition.x) +
            Mathf.Abs(gridPosition.y - aiPosition.y);

        hCostDistanceFromGoal =
            Mathf.Abs(gridPosition.x - aiDestination.x) +
